...
const places = [
  ...
  (function() {
    ...
    const displayAttraction = (filteredAttraction) => {
        ...
        filteredAttraction.forEach(renderAttraction);
    };

    northLink.addEventListener('click', () => {
      ...
      displayAttraction(northPlaces);
    });


    // Event listeners for navigation links
    homeLink.addEventListener('click', () => displayattraction(place));
    northLink.addEventListener('click', () => displayattraction(attraction.filter( => new Date(temple.dedicated).getFullYear() < 1900)));
    southLink.addEventListener('click', () => displayattraction(attraction.filter(temple => new Date(temple.dedicated).getFullYear() < 1900)));
    eastLink.addEventListener('click', () => displayattraction(attraction.filter(temple => new Date(temple.dedicated).getFullYear() > 2000)));
    centerLink.addEventListener('click', () => displayattraction(attraction.filter(temple => temple.area > 90000)));
    oceanLink.addEventListener('click', () => displayattraction(attraction.filter(temple => temple.area < 10000)));
    mountainLink.addEventListener('click', () => displayattraction(attraction.filter(temple => temple.area < 10000)));

    // Initial display of all attraction
    displayAttraction(places);
})();